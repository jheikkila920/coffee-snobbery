...
Plans:
- [x] 14-01-PLAN.md — B1 last-admin guard subquery COUNT (keep FOR UPDATE) + B4 dead self-demote guard removal + multi-admin regression tests (users.py)
- [x] 14-02-PLAN.md — S1 SSRF resolve-validate gate (`_assert_public_host`) on both fetchers + private-IP/mapped/DNS test suite (ai_service.py)
- [x] 14-03-PLAN.md — B2 nightly `nightly_session_sweep` job (03:00, idempotent → 3 jobs) + sweep tests (scheduler.py)
- [x] 14-04-PLAN.md — S4 `/search` 100-char cap + `SEARCH_LIMIT` rate limit + over-long-query test (search.py, rate_limit.py)
